...
public interface ProductionInfoDao
	...
	default List<ProductionInfo> findByMutipleParams(SearchDictionaryList vo, JPQLEnableRepository repository,
	...
	default String mutipleParams(SearchDictionaryList vo, String queryJPQL) {
		...
		if (status != null) {
			queryJPQL = queryJPQL.concat(" and p.status = " + status);
		}
		...
		if (activityProvinceCode != null && activityProvinceCode.size() > 0) {
			...
			activityProvinceCodeString = activityProvinceCodeString.substring(0,
					activityProvinceCodeString.length() - 1);
		...
		if (orderSimulatePriceCode != null && orderSimulatePriceCode.size() > 0) {
			...
			orderSimulatePriceCodeString = orderSimulatePriceCodeString.substring(0,
					orderSimulatePriceCodeString.length() - 1);
